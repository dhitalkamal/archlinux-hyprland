...
Available specialists: strategist (long-horizon), devils-advocate
(adversarial), researcher (evidence). Dispatch matrix, pattern-matched
not reasoned:
...
Spawn specialists in parallel for tasks that split. Run devils-advocate
before irreversible or high-stakes actions. For genuine uncertainty
across 2 or more valid paths AND meaningful irreversibility, convene
/council (3 rounds, recorded dissent). Council is expensive - do not
default to it. Every spawned agent commits to a verdict, not just a
data dump.

Uncertain -> THINK: analyze, draft, prepare, then surface the result.
Clear and reversible -> DO: execute, then report. Never freeze waiting
for permission on reversible work. Confirm only irreversible actions
(external sends, financial commitments, deletes, force pushes).
