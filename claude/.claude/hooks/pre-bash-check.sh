  ...
  if [ -n "$BRANCH" ]; then
    case "$BRANCH" in
      feat/*|fix/*|bug/*|chore/*)
        ...
        ;;
      *)
        block "Commits allowed only on feat/<slug>, fix/<slug>, bug/<slug>, or chore/<slug>. Current branch: ${BRANCH}. Create one: git checkout develop && git checkout -b feat/<slug>"
        ;;
    esac
  fi
fi
